## ---------------------------
##
## Script name: 02_run_analyses.R
##
## Purpose of script: This script contains all the analyses of this study.
## Make sure to choose read_fitted <- TRUE if you do not want to repeat the
## whole machine learning fitting: as random forests fitted to reflectance data ---
...
# Function to average several matrices
...
# Scale the principal components to unit variance (they are centered by construction)
...
# Significant islands based on the results of one particular analysis
islands <- read_csv("results/group_comparisons/machine_learning/PC/randomforest/summary.csv") %>%
  filter(pvalue < 0.05) %>%
  select(island) %>%
  c() %>%
  first()
...
P <- D %>%
  ...
  mutate(wilcoxon = map(data, function(X) {
    ...
    X <- X %>%
      group_by(longitude, latitude, habitat) %>%
      nest()
